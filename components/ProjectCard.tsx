...
export default function ProjectCard({ project }: { project: Project }) {
  return (
    <motion.article
      whileHover={{ y: -6, boxShadow: "0 20px 40px -12px rgba(21,24,31,0.18)" }}
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-5 border-t border-line pt-4">
          ...
          {project.githubUrl2 && (
            <a href={project.githubUrl2} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-sm text-ink transition-colors hover:text-signal">
              <GithubIcon className="h-4 w-4" /> Backend
            </a>
          )}
          {project.liveUrl && (
          ...
          {project.androidUrl && (
            <a href={project.androidUrl} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-sm text-ink transition-colors hover:text-signal">
              ▶ Play Store
            </a>
          )}
          {project.iosUrl && (
            <a href={project.iosUrl} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-sm text-ink transition-colors hover:text-signal">
               App Store
            </a>
          )}
